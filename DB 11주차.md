...
- FD는 어떤 릴레이션들이 합법적인지 제약 조건으로 정하기 위해 사용한다 
...
![IMG_3360](https://github.com/user-attachments/assets/aad85a4f-5441-43a5-a180-c5c6d9b0a84e)
...
어떤 릴레이션 R이 BCNF에 속하려면, 그 릴레이션에 존재하는 모든 함수적 종속성 α → β에 대해 다음 중 하나라도 만족해야 한다
...
좌변(dept_name)은 후보키가 아님, **BCNF 위반**
...
기준: dept_name → building, budget 에서 **왼쪽에 있는 dept_name이 기준** , 일반화하면, **α → β에서 α를 기준으로 분해**
...
이렇게 BCNF로 분리하면, **데이터 중복을 줄이고 이상(anomalies)을 제거하여 데이터의 일관성과 무결성을 보장**
